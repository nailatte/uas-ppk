...
class GetPeriodeByOrmawaUseCase @Inject constructor(
    private val repository: PeriodePendaftaranRepository
) {
    operator fun invoke(ormawaId: Long): Flow<ResultState<List<PeriodePendaftaranResponse>>> {
        return repository.getByOrmawa(ormawaId)
    }
}
